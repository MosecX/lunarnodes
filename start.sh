...
if [ "$ARCH" = "x86_64" ]; then
  ...
  printf "Arquitectura de CPU no compatible: ${ARCH}"
  exit 1
fi
...
display_resources() {
	echo -e " INSTALLER OS -> ${RED} Ubuntu 20.04.5 LTS ${RESET_COLOR}"
	echo -e ""
    ...
    echo -e " PUERTO PRIMARIO -> ${BOLD_GREEN}${SERVER_PORT}${RESET_COLOR}"
    echo -e " PUERTOS ADICIONALES -> ${BOLD_GREEN}${P_SERVER_ALLOCATION_LIMIT}${RESET_COLOR}"
    echo -e " UUID DEL SERVIDOR -> ${BOLD_GREEN}${P_SERVER_UUID}${RESET_COLOR}"
    echo -e " UBICACIÓN -> ${BOLD_GREEN}${P_SERVER_LOCATION}${RESET_COLOR}";
